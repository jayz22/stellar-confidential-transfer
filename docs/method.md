...
output $(C, D)$ is the encrypted value. Note: $C$ is the encrypted message, independent of the public key $Y$. $D$ is the decryption handle, independent on the message being.

## Decryption
$$
mG = C - sk * D
$$
the solve the discrete log to get $m$. Note: solving discrete log is slow, therefore we need the message size to be small.


Twisted ElGamal encryption is addititive homomorphic. Consider two messages $m_1$ and $m_2$, encrypted with two different randomness $r_1$ and $r_2$
...
Linear homomorphism means the messages can be added in the encrypted space. One important detail is that the randomness is also added. Which means if $m_1$ and $m_2$ are different parties, neither one knows the randomness that encrypts the message $m_1+m_2$. This does not matter for decrypting the message (and auditing), but it does matter for zero-knowledge proving of such a message (such as used in a range proof). 

Randomness reuse: when generating cyphertext on the same message, we can reuse the randomness, such that the $C$ portion is the same, only $D$ is different with different encryption keys. This reduces the size of the transaction.
For security analysis of randomness reuse, refer to 

## $\Sigma$-protocol

$\Sigma$-protocol is a method to prove some relations given some public inputs and secrets (witnesses) while preserving zero-knowledge on the secrets. At a high level it contains the following steps 1. the prover generates commitment to some secret witnesses 2. the verifier generates a random challenge 3. the prover responds to the challenge by computing some "signatures" 4. the verifier is able to verify the original statements by verifying the "signatures", using the commitments instead of the witnesses, thus preserving zero-knowledge. TODO: citation.

It is a protocol not a proof. A proof contains public inputs and witnesses and the relations that needs to be proven.
...
1. Prover pick a random value $a$, $b$, computes $A = aG+bH$ and $\forall i=1..N$, $B_i = b*Y_i$ sends $A$, $B_i$ to the verifier.

2. Verifier: generates a random challenge $e$ and send it to the prover

3. Prover computes $z_1 = a + e*m$  and $z_2 = b + e*r$, sends $z_1$, $z_2$ to the verifier

4. Verifier computes and evaluates the following equality relations: 

    - $z_1G+z_2H \equiv A+eC$
    - $\forall i=0..N$, $z_2Y \equiv B_i + eD$

    if any of them is false, return `false`, otherwise return `true`

### Valid amount proof

Proves that the amount $m$ is correctly encoded, and is in the right range, i.e. $\mathbb{L}_{right} \subset \mathbb{L}_{enc} \bigwedge \mathbb{L}_{range}$. 

$\mathbb{L}_{range}$ is the range proof that requires bullet proof, we will not describe it here (TODO: citation). $\mathbb{L}_{enc}$ is a sigma proof very similar to the above $\mathbb{L}_{equal}$ with only one party.
...
If the sender knew the randomness $\tilde{r}$, then proving can be done with $\mathbb{L}_{right}$ above. However, the sender doesn't know that, because the randomness is the sum of all randomness from previous transactions (some coming from other senders). So the sender needs to decrypt the amount first, then commit it with a new randomness $r'$, in order to prove the new balance's range with Bulletproof. 
...
3. The secret key correspond to the sender's public key
$$
Y = sk^{-1}H
$$
...
Decryption of the right amount: $\tilde{C} - C^{*} = bG + sk*(\tilde{D} - D_s^{*})$
...
Statements that needs to be proven in a transfer Sigma proof. 
...
$$ C' = bG + r'H $$
$$\tilde{C} - C^{*} = bG + sk*(\tilde{D} - D_s^{*})$$


3. The decryption key correspond to sender's encryption key
$$
Y_s = sk_s^{-1}H
$$
...
2. We do not need to provide range proof for the recepient's new balance, because that's protected by the range proof on the amount, and the pending balance counter mechanism.
...
#### Setup
Assume without losing generality, the balance contains two chunks that are offset by 16 bits (initially when created, each chunk was 16-bits), and little-endian. Any any given point (after many transfers), the balance cyphertext contains two chunks $(\tilde{C_0}, \tilde{C_1})$ and decryption handles $\tilde{D}$ and $\tilde{D_A}$ (for the auditor public key $Y_A$, assuming there is only one), under some randomness $\tilde{r}$ (not known to anyone).

After the balance is normalized, it contains two chunks $(C_0, C_1)$ and and decryption handles $D$ and $D_A$ under some new randomness $r$.
...
$$\tilde{C_0} + 10^{16} \tilde{C_1} - C_0 - 10^{16} C_1 = 0*G + (1+10^{16})sk*(\tilde{D}-D) $$
...
In addition, two range proofs are needed that both $b_0$ and $b_1$ fits within 16 bits.
